...
Public Partial Class FormCategorias
	...
	Sub Bt_salvarClick(sender As Object, e As EventArgs)
		...
		If autenticaForm = True Then	
			If v_update = 1 Then
				categoria.Nome = txt_nome.Text
				CadCategoria.AdicionarCategoria(categoria)
				LimpaCampos
				
			ElseIf v_update = 2 Then
				categoria.IdCategoria = CInt (txt_id.Text)
				categoria.Nome = txt_nome.Text
				CadCategoria.EditarCategoria(categoria)			
				LimpaCampos
			End If
		Else
			Exit Sub
	...
	Sub Txt_LocalizarKeyDown(sender As Object, e As KeyEventArgs)
		If e.KeyCode = Keys.Enter Then
			...
			dt = CadCategoria.LocalizarCategoria(txt_Localizar.Text)
			bs.DataSource = dt
			dgv_Categorias.DataSource = bs
			
			dgv_Categorias.Columns(1).HeaderText = "NOME CATEGORIA"
	...
	Sub Dgv_CategoriasKeyDown(sender As Object, e As KeyEventArgs)
		...
		If e.KeyCode = Keys.Enter Then
			If dgv_categorias.RowCount < 1 Then 
				MsgBox("Escolha uma categoria válida",vbOKOnly,"ATENÇÃO")
				txt_Localizar.Focus
			Else
	...
	Sub Bt_excluirClick(sender As Object, e As EventArgs)
		...
		Else
			If MsgBox("Confirme a exclusão!",vbYesNo , "Excluir")= vbYes Then
				CadCategoria.ExcluirCategoria(txt_id.Text)
				LimpaCampos
			Else
				Exit Sub
			End If		
